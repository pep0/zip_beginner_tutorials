...
module txuart(i_clk, i_wr, i_data, o_uart_tx, o_busy);
	parameter	[11:0]	CLOCKS_PER_BAUD = 11'd1250;
	input	wire		i_clk;
	input	wire		i_wr;
...
`endif
	...
	// Constraining for Induction
	
	always @(*)
	case(state)
	IDLE:		assert(lcl_data == 9'h1ff);
	START:		assert(lcl_data == {fv_data, 1'b0});
	BIT_ZERO:	assert(lcl_data == {1'b1 ,fv_data});
	...
	BIT_SEVEN:	assert(lcl_data == {8'hFF ,fv_data[7]});
	default: 	assert(0);
	endcase
